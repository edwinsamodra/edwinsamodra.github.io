  ...
  switch (status) {
    case 'Done':
      return {
        ...
        badge: 'bg-emerald-50 text-emerald-700 border border-emerald-200 dark:bg-white/10 dark:text-white dark:border-white/20',
        icon: 'i-lucide-circle-check'
      }
    case 'In Progress':
      return {
        text: 'text-amber-700 dark:text-zinc-300',
        badge: 'bg-amber-50 text-amber-700 border border-amber-200 dark:bg-zinc-800/40 dark:text-zinc-300 dark:border-zinc-700/50',
        icon: 'i-lucide-clock'
      }
    default: // To Do
      return {
        text: 'text-zinc-600 dark:text-zinc-500',
        badge: 'bg-zinc-100 text-zinc-600 border border-zinc-200 dark:bg-zinc-950 dark:text-zinc-400 dark:border-zinc-800',
...
<template>
  <UContainer class="py-12 space-y-16 animate-in fade-in slide-in-from-bottom-4 duration-1000">
    ...
    <header class="space-y-6 max-w-3xl">
      <div class="space-y-2">
        <h1
          class="text-4xl font-extrabold tracking-tight text-zinc-900 dark:text-white sm:text-6xl bg-gradient-to-r from-zinc-900 via-zinc-700 to-zinc-500 dark:from-white dark:via-zinc-200 dark:to-zinc-500 bg-clip-text text-transparent"
        >
          Learning Labs
        </h1>
        <p class="text-lg text-zinc-600 dark:text-zinc-400 leading-relaxed pt-2">
    ...
    <section class="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
      <div
        class="relative overflow-hidden bg-zinc-50 dark:bg-zinc-900/40 backdrop-blur-md border border-zinc-200 dark:border-zinc-800/80 rounded-2xl p-6 transition-all hover:bg-zinc-100/80 dark:hover:bg-zinc-900/60 hover:border-zinc-300 dark:hover:border-white/20 hover:shadow-md dark:hover:shadow-[0_0_30px_rgba(255,255,255,0.02)]"
      >
        <div class="flex items-center justify-between mb-4">
          <span class="text-sm font-medium text-zinc-500 uppercase tracking-wider">Total Progress</span>
          <UIcon
            name="i-lucide-layers"
            class="w-5 h-5 text-zinc-500 dark:text-zinc-400 shrink-0"
          />
        </div>
        <div class="flex items-baseline gap-2">
          <span class="text-3xl font-bold text-zinc-900 dark:text-white">{{ stats.progressPercent }}%</span>
          <span class="text-xs text-zinc-500">completed</span>
        </div>
        <!-- Progress Bar -->
        <div class="w-full bg-zinc-200 dark:bg-zinc-800 rounded-full h-1.5 mt-4 overflow-hidden">
          <div
            class="bg-zinc-900 dark:bg-white h-1.5 rounded-full transition-all duration-1000"
      ...
      <div
        class="bg-zinc-50 dark:bg-zinc-900/40 backdrop-blur-md border border-zinc-200 dark:border-zinc-800/80 rounded-2xl p-6 transition-all hover:bg-zinc-100/80 dark:hover:bg-zinc-900/60 hover:border-zinc-300 dark:hover:border-white/20 hover:shadow-md dark:hover:shadow-[0_0_30px_rgba(255,255,255,0.02)]"
      >
        <div class="flex items-center justify-between mb-4">
        ...
        <div class="flex items-baseline gap-2">
          <span class="text-3xl font-bold text-zinc-900 dark:text-white">{{ stats.done }}</span>
          <span class="text-xs text-zinc-500">/ {{ stats.total }} total</span>
        </div>
        <p class="text-xs text-zinc-600 dark:text-zinc-400 mt-4 leading-relaxed">
      ...
      >
        ...
        <div class="flex items-baseline gap-2">
          <span class="text-3xl font-bold text-zinc-900 dark:text-white">{{ stats.inProgress }}</span>
      ...
      >
        ...
        <div class="flex items-baseline gap-2">
          <span class="text-3xl font-bold text-zinc-900 dark:text-white">{{ stats.todo }}</span>
    ...
    <section class="space-y-6 bg-zinc-50 dark:bg-zinc-900/20 border border-zinc-200 dark:border-zinc-800/80 p-6 rounded-2xl">
      <!-- Search & Status Row -->
      <div class="flex flex-col lg:flex-row gap-4 items-center justify-between">
        ...
        <div class="relative w-full lg:w-96">
          <UInput
            ...
            size="md"
            class="w-full text-zinc-800 dark:text-zinc-200"
        ...
        <div class="flex items-center gap-3 w-full lg:w-auto justify-start lg:justify-end">
          ...
          <div class="flex bg-zinc-200/60 dark:bg-zinc-900 p-0.5 rounded-lg border border-zinc-200 dark:border-zinc-800 w-full sm:w-auto">
            <button
              ...
              :class="[
                selectedStatus === status
                  ? 'bg-white dark:bg-zinc-800 text-zinc-900 dark:text-white border border-zinc-300/80 dark:border-zinc-700 shadow-sm'
          ...
          <button
            ...
            :class="[
              selectedPhaseIndex === null
                ? 'bg-zinc-900 text-white dark:bg-white/10 dark:text-white border-zinc-900 dark:border-white/20 shadow-sm'
      ...
      <div
        v-if="searchQuery || selectedPhaseIndex !== null || selectedStatus !== 'All'"
    ...
    <section class="space-y-16">
      ...
      >
        <div class="p-4 rounded-full bg-zinc-200/60 dark:bg-zinc-900 mb-4 border border-zinc-200 dark:border-zinc-800">
          <UIcon
            name="i-lucide-filter"
            class="w-8 h-8 text-zinc-500 dark:text-zinc-600"
          />
        </div>
      ...
      <!-- Phase Container -->
      <div
        v-for="(phase, phaseIdx) in filteredPhases"
        :key="phase.title"
        class="space-y-8 animate-in fade-in duration-500"
      >
        <!-- Phase Section Header (Fully detailed, no truncation) -->
        <div class="space-y-2 border-b border-zinc-200 dark:border-zinc-800/80 pb-6">
          <div class="flex items-center gap-3">
            <div
              class="w-10 h-10 rounded-xl flex items-center justify-center border bg-zinc-100 dark:bg-white/5 text-zinc-800 dark:text-white border-zinc-200 dark:border-white/10 shrink-0"
            >
              <UIcon
                :name="phase.icon"
                class="w-5 h-5"
            ...
            <div>
              ...
              <h2 class="text-2xl font-bold text-zinc-900 dark:text-zinc-150">
                {{ phase.title }}
              </h2>
            </div>
          </div>
          <p class="text-zinc-600 dark:text-zinc-400 max-w-3xl leading-relaxed text-sm">
            {{ phase.description }}
          </p>
        </div>
        ...
        <div class="grid gap-6 md:grid-cols-2">
          <div
            v-for="lab in phase.labs"
            :key="lab.labCode"
            class="group relative overflow-hidden bg-white dark:bg-zinc-900/20 hover:bg-zinc-50 dark:hover:bg-zinc-900/40 border border-zinc-200 dark:border-zinc-800/80 hover:border-zinc-300 dark:hover:border-white/20 hover:shadow-md dark:hover:shadow-[0_0_30px_rgba(255,255,255,0.02)] rounded-2xl p-6 transition-all duration-300 flex flex-col justify-between"
          >
            <!-- Hover Accent Gradient -->
            <div
              class="absolute top-0 right-0 w-32 h-32 bg-gradient-to-br from-zinc-500/5 dark:from-white/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 rounded-bl-full pointer-events-none"
            />
            ...
            <div class="space-y-4">
              ...
              <div class="flex items-center justify-between">
                <span class="text-xs font-mono text-zinc-500">
                  <span class="font-bold text-zinc-700 dark:text-zinc-400">{{ lab.labCode }}</span>
                </span>

                <span
                  class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium shrink-0"
                  :class="getStatusClasses(lab.status).badge"
                >
                  <span
                    v-if="lab.status === 'In Progress'"
                    class="w-1.5 h-1.5 rounded-full bg-amber-500 dark:bg-zinc-400 animate-ping shrink-0"
                  />
                  <UIcon
                    :name="getStatusClasses(lab.status).icon"
              ...
              <!-- Title -->
              <h4 class="text-base font-bold text-zinc-900 dark:text-zinc-200 group-hover:text-zinc-950 dark:group-hover:text-white transition-colors">
                {{ lab.subtopic }}
              </h4>

              <!-- Detail / Practical Guidelines -->
              <p class="text-zinc-600 dark:text-zinc-450 text-sm leading-relaxed grow">
                {{ lab.detail }}
              </p>
            </div>
            ...
            <div
              v-if="lab.evidence || lab.notes"
              class="mt-6 pt-4 border-t border-zinc-200 dark:border-zinc-800/80 space-y-2 text-xs"
            >
              <div
                v-if="lab.evidence"
              ...
              >
                <UIcon
                  name="i-lucide-circle-check"
                  class="w-4 h-4 text-emerald-600 dark:text-white shrink-0"
                />
                <span class="font-semibold text-zinc-800 dark:text-zinc-300">Evidence:</span>
              ...
              >
                <span class="font-semibold text-zinc-800 dark:text-zinc-300">Notes:</span> {{ lab.notes }}
              </div>
            </div>
          </div>
        </div>
